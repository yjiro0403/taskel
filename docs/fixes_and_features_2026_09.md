...
- **Change:**
    ...
    - The edit modal gets a Delete button (with confirmation), so tasks can be deleted from the timeline view (previously only the list view's selection bar could). A routine occurrence is skipped instead of deleted, since deleting its row would only let the virtual occurrence reappear. `deleteTask` accepts `occurrenceDate` like `updateTask`.
    ...
    - A finished block ends exactly at `completedAt` (the 15-minute minimum only pads the rendered height, labels keep the real times, overlap columns use the padded length). The timeline's ■ records at least one minute for a timed run because `actual_minutes` is an integer column: a run stopped within seconds used to round to 0 and fall back to the planned length.
