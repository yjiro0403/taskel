...
describe('routineOccursOn', () => {
    ...
    describe('weekly', () => {
        it('指定曜日のみ出現する（2026-07-06 は月曜）', () => {
            ...
            expect(routineOccursOn(r, '2026-07-06')).toBe(true); // 月
            expect(routineOccursOn(r, '2026-07-07')).toBe(false); // 火
        });
        it('daysOfWeek 未指定なら開始日の曜日で判定', () => {
            // 2026-01-01 は木曜
            const r = makeRoutine({ frequency: 'weekly', startDate: '2026-01-01' });
            expect(routineOccursOn(r, '2026-01-08')).toBe(true); // 翌週の木
            expect(routineOccursOn(r, '2026-01-09')).toBe(false); // 金
        });
    });
    ...
    describe('monthly（月末繰り上げ）', () => {
        it('31日開始でも31日が無い月は月末に出現する', () => {
            ...
            expect(routineOccursOn(r, '2026-02-28')).toBe(true); // 2026-02 は28日まで
